...
const Home = () => {
  ...
  const handlePlayPause = async (song) => {
    try {
      ...
      const response = await axios({
        method: 'GET',
        url: `http://localhost:5000/api/songs/stream/${song._id}`,
        headers: {
          'Authorization': `Bearer ${token}`
        },
        responseType: 'blob'
  ...
  const handleDownload = async (format) => {
    ...
    try {
      ...
      const response = await axios({
        method: 'GET',
        url: `http://localhost:5000/api/songs/stream/${selectedSong._id}?format=${format}`,
        headers: {
          'Authorization': `Bearer ${token}`
        },
        responseType: 'blob',
      ...
      // Use song title for the download filename
      const extension = format.toLowerCase();
      link.setAttribute('download', `${selectedSong.title}.${extension}`);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (playingSong?._id === selectedSong._id) {
        if (audioElement) {
          audioElement.pause();
          audioElement.src = '';
        }
                      ...
                      >
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          <IconButton
                            ...
                            color={playingSong?._id === song._id ? "primary" : "default"}
                          >
